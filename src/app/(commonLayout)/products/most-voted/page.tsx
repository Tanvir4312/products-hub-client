...
const MostVotedProductsPage = () => {
  // We fetch a larger amount for the dedicated page (e.g., 24)
  const { data: products, isLoading, isError } = useMostVotedProducts(24)
    ...
    <div className="max-w-7xl mx-auto px-6 sm:px-8 py-12 sm:py-20 space-y-12">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-2">
          ...
          >
            The ultimate community-ranked leaderboard. These products have received the highest level of engagement and support from our users.
          </motion.p>
        </div>
      </div>
      ...
      {isError ? (
        <div className="py-20 text-center bg-destructive/5 rounded-[3rem] border border-destructive/10">
          ...
          <p className="text-muted-foreground text-sm">Failed to retrieve the leaderboard data.</p>
        </div>
      ) : (
      ...
      {!isLoading && !isError && products?.length === 0 && (
        <div className="py-32 text-center border-2 border-dashed border-border/50 rounded-[4rem] bg-muted/5">
          ...
          <p className="text-muted-foreground text-xl font-bold italic tracking-tight opacity-50">
            The leaderboard is currently empty. Start voting!
          </p>
        </div>
      )}
    </div>
...
export default MostVotedProductsPage
