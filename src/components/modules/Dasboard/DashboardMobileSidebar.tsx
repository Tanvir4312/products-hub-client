...
const DashboardMobileSidebar = ({ dashboardHome, navItems, userInfo }: DashboardMobileSidebarProps) => {
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto bg-white dark:bg-gray-950">
      {/* Logo / Brand */}
      <div className="flex h-16 items-center border-b px-6 border-gray-200 dark:border-gray-800">
      ...
      <ScrollArea className="flex-1 px-3 py-4">
        <nav className="space-y-1">
          {navItems?.map((section, sectionId) => (
            <div key={sectionId}>
              {section?.title && (
                <h4 className="mb-2 px-3 text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">
                  {section?.title}
                </h4>
              )}
