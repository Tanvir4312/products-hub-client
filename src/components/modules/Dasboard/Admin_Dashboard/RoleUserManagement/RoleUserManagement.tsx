...
const RoleUserManagement = () => {
  ...
  const fetchUsers = async () => {
    setIsLoading(true);
    try {
      const res = await getAllUsers(page, limit, searchTerm, sortBy, sortOrder, "USER", statusFilter);
      if (res.success) {
  ...
  }, [page, limit, searchTerm, sortBy, sortOrder, statusFilter]);
    ...
    <div className="p-6 md:p-8 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-zinc-950 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-900 shadow-xl shadow-zinc-200/20 dark:shadow-none relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-5">
          <UserIcon className="w-40 h-40 text-zinc-900 dark:text-white" />
        </div>
        <div className="space-y-2 relative z-10">
          ...
          <p className="text-zinc-500 text-sm font-medium">Managing general platform users and access levels.</p>
        </div>
                  ...
                  <TableCell className="px-8 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2 border-zinc-200 dark:border-zinc-800 shadow-2xl">
                        <DropdownMenuLabel className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400">User Operations</DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        ...
                        <div className="px-3 py-2">
                          ...
                          <div className="grid grid-cols-1 gap-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              disabled={user.status === "ACTIVE" || user.role === "SUPER_ADMIN"}
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              disabled={user.status === "INACTIVE" || user.role === "SUPER_ADMIN"}
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-96 text-center">
                  <div className="flex flex-col items-center justify-center space-y-4 opacity-50">
                    ...
                    <div>
                      <p className="font-black text-zinc-900 dark:text-zinc-100 uppercase tracking-widest text-sm italic">Zero Users Detected</p>
