...
import { IModerator } from "@/types/Dashboard/admin-dashboard-types/moderators-managements.types";
...
const ModeratorManagement = () => {
  ...
  const [moderatorToDelete, setModeratorToDelete] = useState<IModerator | null>(null);
  ...
  const fetchModerators = async () => {
    setIsLoading(true);
    try {
      const res = await getAllModerators(page, limit, searchTerm, sortBy, sortOrder, statusFilter, genderFilter);
      if (res.success) {
  ...
  }, [page, limit, searchTerm, sortBy, sortOrder, statusFilter, genderFilter]);
  ...
  const handleDeleteModerator = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await deleteUser(moderatorToDelete.id, "MODERATOR"); 
      if (res.success) {
  ...
  const getRoleIcon = (role: string) => {
    switch (role) {
      ...
      case "MODERATOR": return <ShieldHalf className="w-4 h-4 text-amber-500" />;
    ...
    <div className="p-6 md:p-8 space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-zinc-950 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-900 shadow-xl shadow-zinc-200/20 dark:shadow-none relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-5">
          <ShieldHalf className="w-40 h-40 text-zinc-900 dark:text-white" />
        </div>
        <div className="space-y-2 relative z-10">
          ...
          <h1 className="text-4xl font-black tracking-tighter text-zinc-900 dark:text-zinc-100 italic">Moderator <span className="text-zinc-400 font-serif">Registry</span></h1>
          <p className="text-zinc-500 text-sm font-medium">Managing platform moderators and content governors.</p>
          <Button 
            onClick={() => setIsCreateModalOpen(true)}
            className="mt-4 h-12 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-black text-[10px] uppercase tracking-widest px-6 transition-all flex items-center gap-2 shadow-xl shadow-amber-500/20 border-none"
          >
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto relative z-10">
          ...
          <Select value={genderFilter} onValueChange={(v) => { setGenderFilter(v); setPage(1); }}>
            ...
            <SelectContent className="rounded-2xl">
              <SelectItem value="all" className="text-[10px] font-black uppercase tracking-widest">All Genders</SelectItem>
        ...
        <Table>
          <TableHeader className="bg-zinc-50/50 dark:bg-zinc-900/50">
            <TableRow className="border-zinc-100 dark:border-zinc-900 hover:bg-transparent h-16">
              ...
              <TableHead className="text-[10px] font-black uppercase tracking-widest text-zinc-400 text-center">Gender</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              moderators.map((moderator) => (
                <TableRow key={moderator.id} className="border-zinc-100 dark:border-zinc-900 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 group transition-all duration-300 h-24">
                  <TableCell className="px-8">
                    <div className="flex items-center gap-4">
                      {moderator.profilePhoto ? (
                        <Image src={moderator.profilePhoto} alt={moderator.name} width={48} height={48} className="w-12 h-12 rounded-2xl object-cover border border-zinc-200 dark:border-zinc-800" />
                      ) : (
                        <div className="w-12 h-12 rounded-2xl bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center font-black text-zinc-400 group-hover:scale-105 transition-transform overflow-hidden uppercase">
                          {moderator.name.substring(0, 2)}
                        </div>
                      )}
                      <div className="min-w-0">
                        <p className="font-bold text-zinc-900 dark:text-zinc-100 truncate">{moderator.name}</p>
                        <div className="flex items-center gap-1.5 mt-0.5 opacity-60">
                          <Mail size={10} className="text-zinc-400" />
                          <p className="text-[10px] font-medium truncate tracking-tight">{moderator.email}</p>
                        </div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className={`p-2 rounded-lg ${moderator.user.role === 'SUPER_ADMIN' ? 'bg-rose-50 dark:bg-rose-950/30' : moderator.user.role === 'ADMIN' ? 'bg-blue-50 dark:bg-blue-950/30' : 'bg-amber-50 dark:bg-amber-950/30'}`}>
                        {getRoleIcon(moderator.user.role)}
                      </div>
                      <span className="text-xs font-black tracking-tighter uppercase">{moderator.user.role}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-center">
                    <span className="text-xs font-black tracking-tighter uppercase text-zinc-500">{moderator.gender || 'N/A'}</span>
                  </TableCell>
                  <TableCell className="text-center">
                    {getStatusBadge(moderator.user.status)}
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2 text-zinc-500">
                      <Calendar size={12} />
                      <span className="text-[11px] font-bold uppercase tracking-tighter">
                        {new Date(moderator.createdAt).toLocaleDateString(undefined, { dateStyle: "medium" })}
                      </span>
                    </div>
                  </TableCell>
                  <TableCell className="px-8 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2 border-zinc-200 dark:border-zinc-800 shadow-2xl">
                        <DropdownMenuLabel className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400">Moderator Operations</DropdownMenuLabel>
                        <DropdownMenuSeparator />
                        ...
                        <div className="px-3 py-2">
                          ...
                          <div className="grid grid-cols-1 gap-1">
                            <Button
                              variant="ghost"
                              size="sm"
                              disabled={moderator.user.status === "ACTIVE" || moderator.user.role === "SUPER_ADMIN"}
                              onClick={() => handleStatusChange(moderator.user.id, "ACTIVE")}
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              disabled={moderator.user.status === "INACTIVE" || moderator.user.role === "SUPER_ADMIN"}
                              onClick={() => handleStatusChange(moderator.user.id, "INACTIVE")}
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              disabled={moderator.user.status === "SUSPENDED" || moderator.user.role === "SUPER_ADMIN"}
                              onClick={() => handleStatusChange(moderator.user.id, "SUSPENDED")}
                        ...
                        <div className="px-3 py-2">
                          ...
                          <Select
                            defaultValue={moderator.user.role}
                            onValueChange={(val: any) => handleRoleChange(moderator.user.id, val)}
                            disabled={moderator.user.role === "SUPER_ADMIN"}
                          >
                        ...
                        <DropdownMenuItem
                          disabled={moderator.user.role === "SUPER_ADMIN"}
                          onClick={() => setModeratorToDelete(moderator)}
                        ...
                        >
                          ...
                          <span className="text-xs font-bold uppercase tracking-tighter">Delete Moderator</span>
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
              ...
              <TableRow>
                <TableCell colSpan={6} className="h-96 text-center">
                  <div className="flex flex-col items-center justify-center space-y-4 opacity-50">
                    ...
                    <div>
                      <p className="font-black text-zinc-900 dark:text-zinc-100 uppercase tracking-widest text-sm italic">Zero Moderators Detected</p>
                      <p className="text-zinc-500 text-[10px] font-bold uppercase mt-1 tracking-tighter">No moderator records matched your current filtering criteria.</p>
                    </div>
                  </div>
                </TableCell>
              </TableRow>
        ...
        <AlertDialogContent className="rounded-[2rem] border-rose-100 dark:border-zinc-900 bg-white dark:bg-zinc-950 shadow-2xl">
          <AlertDialogHeader>
            <AlertDialogTitle className="flex items-center gap-2 text-rose-600 font-black tracking-tight text-2xl italic">
              <Trash2 className="w-6 h-6" /> Delete Moderator?
            </AlertDialogTitle>
            <AlertDialogDescription className="text-zinc-500 font-medium py-2">
              This will permanently remove <span className="font-bold text-zinc-900 dark:text-zinc-100">{moderatorToDelete?.name}</span> from the moderators registry. This action cannot be reversed.
            </AlertDialogDescription>
          </AlertDialogHeader>
