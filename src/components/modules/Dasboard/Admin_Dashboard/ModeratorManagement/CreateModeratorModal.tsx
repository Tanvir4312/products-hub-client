...
import { createModerator } from "@/services/admin-server-action";
...
const CreateModeratorModal = ({ isOpen, onOpenChange, onSuccess }: CreateModeratorModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await createModerator(payload);
      if (res.success) {
        ...
        toast.error(res.message || "Failed to create moderator");
      }
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden rounded-[2rem] border-zinc-200 dark:border-zinc-900 bg-white dark:bg-zinc-950 shadow-2xl">
        <div className="bg-zinc-900 dark:bg-zinc-900 p-8 text-white relative overflow-hidden">
          <div className="absolute top-0 right-0 p-8 opacity-10">
            <ShieldHalf className="w-24 h-24 text-amber-500" />
          </div>
          <DialogHeader className="relative z-10">
            <DialogTitle className="text-3xl font-black tracking-tighter italic flex items-center gap-3">
              <ShieldHalf className="w-8 h-8 text-amber-500" />
              Provision <span className="text-zinc-400 font-serif">Moderator</span>
            </DialogTitle>
            <DialogDescription className="text-zinc-400 font-medium pt-1">
              Onboard a new content governor with moderation capabilities.
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 gap-6">
            <div className="space-y-2">
              ...
              <div className="relative">
                <ShieldHalf className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" />
                <Input
                  required
                  placeholder="Moderator full name (min 3 chars)"
                  value={formData.name}
                  onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                  className="pl-12 h-12 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-amber-500/20 font-medium"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Access Credentials</Label>
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-12 pr-12 h-12 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-amber-500/20 font-medium"
                />
                <button
                  type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Gender Identification</Label>
                <Select
            ...
            <div className="space-y-2">
              <Label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Profile Image URL (Optional)</Label>
              <div className="relative">
          ...
          <div className="pt-4 flex gap-3">
            ...
            <Button
              ...
              type="submit"
              className="flex-[2] h-14 rounded-2xl bg-amber-500 hover:bg-amber-600 text-white font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-amber-500/20"
            >
              {isSubmitting ? (
