...
import { IUser } from "@/types/Dashboard/admin-dashboard-types/users-managements.types";
...
const UserManagement = () => {
  ...
  const fetchUsers = async () => {
    setIsLoading(true);
    try {
      const res = await getAllUsers(page, limit, searchTerm, sortBy, sortOrder, roleFilter, statusFilter);
      if (res.success) {
  ...
  }, [page, limit, searchTerm, sortBy, sortOrder, roleFilter, statusFilter]);
    ...
    <div className="p-6 md:p-8 space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white dark:bg-zinc-950 p-8 rounded-[2.5rem] border border-zinc-200 dark:border-zinc-900 shadow-xl shadow-zinc-200/20 dark:shadow-none relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-5">
          <Users className="w-40 h-40 text-zinc-900 dark:text-white" />
        </div>
        <div className="space-y-2 relative z-10">
          <div className="flex items-center gap-2 px-3 py-1 bg-blue-500/10 text-blue-600 w-fit rounded-lg text-[10px] font-black uppercase tracking-widest border border-blue-500/20">
            <ShieldCheck className="w-3 h-3" />
            Platform Authority
          </div>
          <h1 className="text-4xl font-black tracking-tighter text-zinc-900 dark:text-zinc-100 italic">User <span className="text-zinc-400 font-serif">Registry</span></h1>
          <p className="text-zinc-500 text-sm font-medium">Managing privileges and accessibility across the entire platform ecosystem.</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto relative z-10">
          <div className="relative w-full sm:w-64">
            ...
            <Input
              ...
              className="pl-12 h-12 rounded-2xl bg-zinc-50 dark:bg-zinc-900 border-zinc-200 dark:border-zinc-800 focus:ring-2 focus:ring-blue-500/20 transition-all font-medium"
            />
          </div>

          <Select value={roleFilter} onValueChange={(v) => { setRoleFilter(v); setPage(1); }}>
            <SelectTrigger className="w-full sm:w-40 h-12 rounded-2xl border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-900 text-xs font-black uppercase tracking-widest">
            ...
            <SelectContent className="rounded-2xl">
              ...
              <SelectItem value="MODERATOR" className="text-[10px] font-black uppercase tracking-widest">Moderator</SelectItem>
      ...
      {/* Automated Deletion Policy Notice */}
      <div className="bg-rose-50/50 dark:bg-rose-950/20 border border-rose-200/50 dark:border-rose-900/50 p-4 rounded-2xl flex items-center gap-3">
        <AlertCircle className="w-5 h-5 text-rose-500 shrink-0" />
        <p className="text-xs font-medium text-rose-700 dark:text-rose-300">
          <strong className="font-bold text-rose-900 dark:text-rose-100 uppercase tracking-widest text-[10px] mr-1">Notice:</strong> 
          Any user marked as <span className="font-bold uppercase tracking-wider text-[9px] bg-rose-100 dark:bg-rose-900/40 px-1.5 py-0.5 rounded text-rose-600 mx-0.5">Suspended</span> will be permanently deleted automatically after 1 month of suspension.
        </p>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              users.map((user) => (
                <TableRow key={user.id} className="border-zinc-100 dark:border-zinc-900 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30 group transition-all duration-300 h-24">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-0.5 opacity-60">
                          ...
                          <p className="text-[10px] font-medium truncate tracking-tight">{user.email}</p>
                        </div>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-black tracking-tighter uppercase">{user.role}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="px-8 text-right">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2 border-zinc-200 dark:border-zinc-800 shadow-2xl">
                        <DropdownMenuLabel className="px-3 py-2 text-[10px] font-black uppercase tracking-widest text-zinc-400">Registry Operations</DropdownMenuLabel>
                        <DropdownMenuSeparator />
              ...
              <TableRow>
                <TableCell colSpan={5} className="h-96 text-center">
                  <div className="flex flex-col items-center justify-center space-y-4 opacity-50">
                    ...
                    <div>
                      <p className="font-black text-zinc-900 dark:text-zinc-100 uppercase tracking-widest text-sm italic">Zero Identities Detected</p>
                      <p className="text-zinc-500 text-[10px] font-bold uppercase mt-1 tracking-tighter">No user records matched your current filtering criteria.</p>
                    </div>
                  </div>
                </TableCell>
              </TableRow>
